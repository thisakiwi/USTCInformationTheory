...
class GPTZip:
    def __init__(self):
        ...
        self.BATCH_SIZE = 10
        self.model = AutoModelForCausalLM.from_pretrained("./code/BERT", is_decoder=True, trust_remote_code=True)
        self.tokenizer = AutoTokenizer.from_pretrained("./code/BERT", trust_remote_code=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
                        prog='BertZip',
                        description='Zips files using LLMzip algorithm with BERT')
    ...
    if args.zip is not None:
        ...
        zip_file = os.path.splitext(args.zip)[0] + ".berz"
    
        if args.output_file is not None:
            zip_file = args.output_file
            # 修复：安全判断后缀
            if not zip_file.endswith(".berz"):
                zip_file += ".berz"
        ...
        # 已经是正确的，保留
        if not args.unzip.endswith(".berz"):
            print("This program can only unzip BertZip files (.berz)")
            exit()
        print("Loading BERT model...")
        gpt_zip = GPTZip()
        ...
        print("Usage: BertZip -z input.txt    (compress)")
        print("       BertZip -u input.berz    (decompress)")